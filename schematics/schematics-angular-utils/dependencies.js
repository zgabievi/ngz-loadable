...
function addPackageJsonDependency(tree, dependency, projectPath) {
    const packageJsonAst = _readPackageJson(tree, projectPath);
...
function _readPackageJson(tree, projectPath) {
    const jsonFilePath = projectPath ? `${projectPath}/package.json` : pkgJsonPath;
    const buffer = tree.read(jsonFilePath);
    if (buffer === null) {
