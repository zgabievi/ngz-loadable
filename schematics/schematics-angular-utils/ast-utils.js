...
function addSymbolToNgModuleMetadata(source, ngModulePath, metadataField, symbolName, importPath = null, metadataFieldContent) {
 ...
 */
function addImportToModule(source, modulePath, classifiedName, importPath, metadataFieldContent) {
...
function getRouterModuleDeclaration(source) {
    ...
    return arrLiteral.elements
        ...
        .find(el => el.getText().startsWith('LoadableModule'));
}
...
function addRouteDeclarationToModule(source, fileToAdd, routeLiteral) {
    ...
    const node = scopeConfigMethodArgs[0];
    let routesArr;
    // const routesArg = scopeConfigMethodArgs[0];
    const matchingProperties = getMetadataField(node, 'moduleConfigs');
    const assignment = matchingProperties[0];
    if (assignment.initializer.kind !== ts.SyntaxKind.ArrayLiteralExpression) {
        throw new Error('No moduleConfigs found');
    }
    const routesArg = assignment.initializer;
